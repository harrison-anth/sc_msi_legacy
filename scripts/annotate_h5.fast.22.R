...
if(max(tiny$t_alt_count) > 15){

temp_mat[,gene] <- max(tiny$t_alt_count)/max(tiny$t_depth)
...
maf_mat_trim <- maf_mat2[, colMaxs(maf_mat2,na.rm=TRUE) > 0]
...
saveRDS(new_sample6, paste0('../annotated_h5/',sample_name,'_af.rds'))
