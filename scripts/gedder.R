...
num <- as.numeric(argus[2])

snps <- read.table(paste0("../vartrix_results/",sample_name,".",num,".snv_loci.txt"), header = F)
barcodes <- fread(paste0('../pseudobulk_barcodes/',sample_name,'/',sample_name,'_cluster_',num,'.tsv'),header=FALSE)

snv_matrix <- as.data.frame(as.matrix(readMM(paste0('../vartrix_results/',sample_name,'.',num,'.mmx'))))
