...
maf_mat[cell,gene] <- max(tiny$t_alt_count)

}
}
print(paste0('Finished cell ',cell,'; ', round((cells/nrow(maf_mat) * 100),4), '% complete'))

}

#filter out genes that only have NA values for each cell
maf_mat_trim <- maf_mat[, colMaxs(maf_mat,na.rm=TRUE) > 15]
#maf_mat_trim <- maf_mat[,colSums(is.na(maf_mat))<nrow(maf_mat)]
