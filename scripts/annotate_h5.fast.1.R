...
add_sensor_rna <- function(s_obj,sample_name){
...
add_premsim <- function(s_obj,sample_name){
pre1 <- readRDS(paste0('../premsim/',sample_name,'_premsim.rds'))
s_obj <- AddMetaData(s_obj,pre1$premsim_status,col.name='premsim_status')
s_obj <- AddMetaData(s_obj,pre1$premsim_prob,col.name='premsim_prob')
return(s_obj)
}
...
temp_mat[,gene] <- max(tiny$t_alt_count)
...
saveRDS(new_sample6, paste0('../annotated_h5/',sample_name,'_alt_count.rds'))
