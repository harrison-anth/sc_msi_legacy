...
key <- fread('C:/Users/Harrison Anthony/Desktop/git_repos/single_msi/manifests/combined_key.tsv')

files <- list.files(path='E:/temp/newest_dir/',pattern = '*.rds')
for(file in files){
...
assign(x = 's_obj',value = readRDS(paste0('E:/temp/newest_dir/',file))) 
...
tissue_plot <- DimPlot(s_obj,group.by ='tissue',cols=c(DiscretePalette(length(unique(s_obj$tissue)), palette = NULL, shuffle = FALSE)))
tissue_plot$data$cluster_counts<- as.factor(s_obj$cluster_counts)
...
guides(colour = guide_legend(override.aes = list(size=1.6),ncol=1))+ 
  theme(legend.text=element_text(size=7))),
        print(LabelClusters(tissue_plot,id='cluster_counts',repel = T,box = T,size=3.5
                    )),
...
if(file.exists((paste0('E:/temp/newest_dir/cancer/',file_name,'_cancer.rds'))))
{
assign(x = 's_obj_cancer',value = readRDS(paste0('E:/temp/newest_dir/cancer/',file_name,'_cancer.rds'))) 



plots2 <-  ggarrange(
print(DimPlot(s_obj_cancer)+ggtitle(paste0('Clusters of cells (cancer)','; n=',length(s_obj_cancer$pan_cancer_cluster)))),
#DimPlot(s_obj_cancer,group.by = 'classification_confidence')+ggtitle(paste0('Classification #confidence'))
#,
...
print(DimPlot(s_obj_cancer,group.by ='tissue')+ggtitle('Tissue contribution')),
nrow=1,ncol=1
)
